...
def train(S,A,returns, old_log_probs):
    ...
    values = V(S).flatten()
    delta = (returns - values).detach().clone()
    valueObjective = -((GAMMA**n) * delta * values).sum()
    valueObjective.backward()
    OPT1.step()
    # PPO 
    ...
    for i in range(POLICY_TRAIN_ITERS):
        ...
        alpha, beta = diff * delta, torch.clip(diff, 1-CLIP_PARAM, 1+CLIP_PARAM) * delta
        policyObjective = -(torch.minimum(alpha, beta)).sum()
        policyObjective.backward()
        OPT2.step()
